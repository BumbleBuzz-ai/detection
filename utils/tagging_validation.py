...
## opening class list 
Df_classes = pd.read_csv('utils/TaggingCategory.csv')
...
# Load label
with open('utils/class_labels_indices.csv', 'r') as f:
...
def subset_probas(Df,search_labels):
    ...
    for curlabel in search_labels:
        try:
            ind_list.append(int(np.argwhere([c==curlabel for c in labels]).squeeze()))
